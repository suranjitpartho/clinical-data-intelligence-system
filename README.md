...
|---|---|
| `GROQ_API_KEY` | https://console.groq.com → Sign up → API Keys → Create key |
| `GITHUB_CLIENT_ID` | From the OAuth app you created above |
| `GITHUB_CLIENT_SECRET` | From the OAuth app you created above |
| `JWT_SECRET` | Auto-generated by `openssl rand -hex 32` in `.env.example`. Keep as-is. |
| `LANGFUSE_SECRET_KEY` | (Optional) https://cloud.langfuse.com → Project Settings → API Keys |
| `LANGFUSE_PUBLIC_KEY` | (Optional) Same page as above |
