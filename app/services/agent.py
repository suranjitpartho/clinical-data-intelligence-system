...
class ClinicalAgent:
    ...
    Bypasses LangGraph internal state issues on Python 3.9.
    """
    
    def classify_intent(self, query: str) -> str:
        prompt = f"System: Classify the medical query into 'SQL' (structured data like counts, lists, billing) or 'RAG' (qualitative medical symptoms/notes). Reply with ONLY the word.\nHuman: {query}"
        response = llm.invoke(prompt)
        return response.content.replace("<|eot_id|>", "").strip().upper()

    def generate_sql(self, query: str) -> str:
        prompt = f"System: You are an expert SQL engineer. Write a PostgreSQL query using this exact schema:\n- departments (id, name, location)\n- staff (id, first_name, last_name, role, email, department_id)\n- patients (id, nhi_number, first_name, last_name, gender)\nReturn ONLY the raw SQL code. No markdown formatting.\nHuman: {query}"
        response = llm.invoke(prompt)
        return response.content.replace("<|eot_id|>", "").replace("```sql", "").replace("```", "").strip()
    ...
    def run_query(self, query: str) -> dict:
        ...
        try:
            ...
            # 2. Synthesize Answer
            synth_prompt = f"System: You are a professional medical consultant. Summarize this data to answer accurately.\nQuery: {query}\nData: {data}"
